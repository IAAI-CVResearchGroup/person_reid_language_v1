...
class SENet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, include_top=True, use_l2_norm=False):
        self.inplanes = 64
    ...
    def forward(self, x, use_l2_norm=True, use_bn = True):
...
def load_state_dict(model, fname, include_top=False):
    """
    ...
    for name, param in weights.items():
        if name in own_state:
            ...
            if include_top==False and name.startswith("fc"):
                print("Passsing the fc:{}".format(name))
                continue
            try:
                own_state[name].copy_(torch.from_numpy(param))
            except Exception:
                raise RuntimeError('While copying the parameter named {}, whose dimensions in the model are {} and whose '\
                                   'dimensions in the checkpoint are {}.'.format(name, own_state[name].shape, param.size))
        else:

...
def eval_cuhk03(distmat, q_pids, g_pids, q_camids, g_camids, max_rank, N=100):
    ...
    """
    num_q, num_g = distmat.shape #(965, 965)
    if num_g < max_rank:
        max_rank = num_g
        print("Note: number of gallery samples is quite small, got {}".format(num_g))
    indices = np.argsort(distmat, axis=1) #(965, 965) 按行排序，返回行下标

    matches = (g_pids[indices] == q_pids[:, np.newaxis]).astype(np.int32) # q_pids[:, np.newaxis]--(965, 1)
    ...
    for q_idx in range(num_q):
        ...
        order = indices[q_idx] #(965,)
        remove = (g_pids[order] == q_pid) & (g_camids[order] == q_camid)
        keep = np.invert(remove) #(965,)

        # compute cmc curve
        orig_cmc = matches[q_idx][keep] # binary vector, positions with value 1 are correct matches 找到当前q对应的g(已按照距离排序)
        if not np.any(orig_cmc):
            # this condition is true when query identity does not appear in gallery
            continue

        kept_g_pids = g_pids[order][keep] #留下非当前id的g,维度可能为(960,0)

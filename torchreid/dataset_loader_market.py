...
def read_sentence(img_path):
    ...
    ID = int(img_names[0])
    key_image = img_name
    path = root_path + 'CUHK-PEDES/imgs/'+img_name
    caption_raw = ''
    if not os.path.exists(path) and ID != 0:
        key_image_list = glob.glob(root_path + 'CUHK-PEDES/imgs/Market/'+ '%04d'%ID + '*.jpg')
        key_image = random.choice(key_image_list).replace(root_path + 'CUHK-PEDES/imgs/','')
    if ID!=0:

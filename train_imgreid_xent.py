...
def main():
    ...
    transform_train = T.Compose([
        T.Random2DTranslation(args.height, args.width),#random crop
    ...
    # 建立model
    # args.arch='resnet50':载入backbone为resnet50的ResNet50
    print("Initializing model: {}".format(args.arch))
    model = models.init_model(name=args.arch, num_classes=dataset.num_train_pids, batchsize = args.test_batch, loss={'xent'}, use_gpu=use_gpu)
...
def test(model, queryloader, galleryloader, use_gpu, ranks=[1, 5, 10, 20], return_distmat=False):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (imgs, pids, camids, caps) in enumerate(queryloader):
            if use_gpu: imgs, caps = imgs.cuda(), caps.cuda()
                              
            end = time.time()
            features = model(imgs, caps.float())
            batch_time.update(time.time() - end)
